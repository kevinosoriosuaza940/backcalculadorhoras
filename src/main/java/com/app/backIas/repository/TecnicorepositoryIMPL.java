...
public class TecnicorepositoryIMPL implements ITecnicorepository {
    ...
    private final String queryHoras = "select ST.id_tecnico, S.numero_semana, SUM(H.horas_normales_extras) as horas_normales_extras, SUM(H.horas_dominicales) as horas_dominicales, SUM(H.horas_nocturnas_extras) as horas_nocturnas_extras,  SUM(H.horas_nocturnas) as horas_nocturnas,SUM(H.horas_dominicales_extras) as horas_dominicales_extras, SUM(H.total_horas) as total_horas from Horas H inner join Dia D on H.id_horas = D.id_dia inner join Semana S on D.id_dia = S.id_semana inner join ServicioTecnico ST on ST.id_semana = ST.id_semana where id_tecnico = :id_tecnico and numero_semana = :numero_semana";
    ...
    public List postHoras (PostDataDto post) throws Exception {
        try {
            Query query = em.createNativeQuery(queryInsert, PostDataDto.class);
